...
echo -e "\e[95mTAG is set to ${TAG}\e[0m"

# Enable Cloudbuild API
echo -e "\e[95mEnabling Cloudbuild API in ${PROJECT_ID}\e[0m"
gcloud services enable cloudbuild.googleapis.com storage.googleapis.com cloudresourcemanager.googleapis.com
...
[[ "${DESTROY}" != "true" ]] && gcloud builds submit --config=builds/infra.yaml --substitutions=_TAG=${TAG},_PROJECT_ID=${PROJECT_ID} --async
[[ "${DESTROY}" == "true" ]] && gcloud builds submit --config=builds/infra_destroy.yaml --substitutions=_PROJECT_ID=${PROJECT_ID},_TAG=${TAG} --async
